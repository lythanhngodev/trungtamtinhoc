...
$tenkhoa = '';
$qr_hv = $kn->query("SELECT hv.IDHV,hv.HO, hv.TEN, hv.NGAYSINH, hv.GIOITINH, hv.NOISINH, hv.CMND, hv.MSSV,kh.TENKHOA,l.TENLOP FROM khoahoc kh LEFT JOIN khoahoc_lop khl ON kh.IDKH=khl.IDKH LEFT JOIN lop l ON khl.IDL=l.IDL LEFT JOIN hocvien_lop hvl ON l.IDL=hvl.IDL LEFT JOIN hocvien hv ON hvl.IDHV=hv.IDHV WHERE khl.IDKH='$khoahoc' AND hvl.CAMTHI=b'0' ORDER BY l.TENLOP ASC;"); ?>
<table id="banghocvien" class="table table-hover table-bordered display nowrap" style="width: 100%">
    <thead>
        <tr class="text-center">
        	<th style="width: 40px;"><input type="checkbox" class="text-center checkall"></th>
            <th>Họ tên</th>
            <th>Ngày sinh</th>
            <th>Giới tính</th>
            <th>Lớp</th>
...
while ($row = mysqli_fetch_assoc($qr_hv)) { ?>
    	<tr>
    		<td ly='stt' mahv="<?php echo $row['IDHV'] ?>" class="text-center"><input type="checkbox" class="text-center"></td>
...
<center><div class="col-md-12 khungbtn">
    <button class="btn btn-primary luuthongtin"><i class="fas fa-arrow-right"></i></button>
</div>
<br>
</center>