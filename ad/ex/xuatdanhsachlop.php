...
if (!isset($_SESSION['_checkpage'])) {echo "<h2>Đâu dễ phá vậy</2>";die();}
	...
	$kn = new clsKetnoi();
	$idl = intval($_GET['idl']);
	$qr_l = $kn->query("SELECT l.TENLOP,kh.TENKHOA,kh.TGBATDAU,kh.TGKETTHUC,l.CBGD,l.PHONG,pc.TENCB FROM lop l LEFT JOIN khoahoc_lop khl ON l.IDL=khl.IDL LEFT JOIN khoahoc kh ON khl.IDKH=kh.IDKH LEFT JOIN phanconggiangday pc ON l.MALOP=pc.MALOP WHERE l.IDL = '$idl';");
...
<body style="width: 25.7cm">
	<div class="Section1">
		<div style="margin: 0;box-shadow: 0;font-size: 18px;">
			...
			<p style="text-align: center;">Mã lớp: <?php echo $row['TENLOP'] ?>, CBGD: <?php echo $row['TENCB'] ?>, Phòng: <?php echo $row['PHONG'] ?></p>
