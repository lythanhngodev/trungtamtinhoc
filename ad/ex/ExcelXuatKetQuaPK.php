...
$tenfile = "BM-IC-25-00 - KQPK ".date('d-m-Y');
...
$diemkhoa = $kn->query("
SELECT DISTINCT dh.SBD,CONCAT(hv.HO,' ',hv.TEN) AS HOTEN, hv.NGAYSINH,hv.GIOITINH, hv.NOISINH,pk.TENPK,dk.THILT,dk.THITH,dk.DIEMLTPK,dk.DIEMTHPK,dh.DIEMLT,dh.DIEMTH
...
for ($i=0; $i < count($ds); $i++) { 
    ...
    $sheet->setCellValue("I".$dong,$ds[$i]['DIEMLTPK']);
    $sheet->setCellValue("J".$dong,$ds[$i]['DIEMTH']);
    $sheet->setCellValue("K".$dong,$ds[$i]['DIEMTHPK']);
    $sheet->setCellValue("L".$dong,$ds[$i]['DIEMLTPK']+$ds[$i]['DIEMTHPK']);
    $sheet->setCellValue("M".$dong,($ds[$i]['DIEMLTPK']>=5&&$ds[$i]['DIEMTHPK']>=5)?"Đạt":"Không đạt");
    $dong++;
}
...
$sheet->getStyle('E7:M'.($dong-1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$sheet->getStyle('E7:M'.($dong-1))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
