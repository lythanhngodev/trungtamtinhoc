	...
	<div class="row">
		<div class="col-md-12">
			<div class="card">
				<div class="card-body">
	                <h4>LỆ PHÍ THI</h4>
	                <h6>Quản lý lệ phí thi học viên theo đợt thi</h6>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="row">
		<div class="col-md-12" style="padding: 0">
			<div class="col-md-12">
				<div class="card">
					<div class="card-body">
						<div class="col-md-12"><p>(*) Sau khi nhập dữ liệu lệ phí thi từ Excel vui lòng chọn mục đợt thi bên dưới để xem chi tiết lệ phí.</p></div>
					<div class="form-group col-md-3" style="float: left;">
						<label><b>Chọn đợt thi</b></label>
						<select class="form-control" id="chondotthilay">
							<option value="0">--- Chọn đợt thi ---</option>
							<?php 
							$lp = laydanhsachdangkyduthi();
							while ($row = mysqli_fetch_assoc($lp)) { ?>
			...
			<div class="card">
				<center><br>
					<div class="form-group col-md-3" id="khungkhoahoc">
						<label><b>Xem chi tiết học phí từ đợt thi</b></label>
						<select class="form-control" id="chondotthi">
							<option value="0">--- Chọn đợt thi ---</option>
							<?php 
							$lp = laydanhsachdangkyduthi();
							while ($row = mysqli_fetch_assoc($lp)) { ?>
...
document.getElementById('lephithi').classList.add("active");
...
$('#chondotthi,#chondotthilay').select2({
  width: '100%'
});
...
$(document).on('change','#chondotthi',function(){
	if($(this).val()=='0'){
	...
	var dotthi = $(this).val();
	$.ajax({
		url: 'aj/ajLaydanhsachlephithitheodotthi.php',
		type: 'POST',
		beforeSend: function () {
                tbinfo("Vui lòng chờ...");
            },
...
$(document).on('click','.luuthongtin',function(){
	...
	$.ajax({
		url: 'aj/ajLuuthongtinphithi.php',
		type: 'POST',
		data: {
			bhv:bhv,
...
$(document).on('click','#laydulieu',function(){
	...
	if (type==match[0] || type==match[1]) {
	    ...
	    form_data.append('dotthi', dotthi);
        $.ajax({
            url: './aj/ajLaydulieulephithi.php', // gửi đến file upload.php
            dataType: 'text',
            cache: false,
            contentType: false,
            processData: false,
