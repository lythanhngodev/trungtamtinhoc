...
<?php 
	function laykhoahoc(){
		$kn = new clsKetnoi();
		$query = "SELECT kh.IDKH,kh.TENKHOA,kh.TGBATDAU,kh.TGKETTHUC,kh.HOANTHANH, kh.LOAIKHOA,(SELECT COUNT(*) FROM khoahoc_lop kl WHERE kl.IDKH=kh.IDKH) as 'SOLOP' from khoahoc kh WHERE kh.HOANTHANH=b'0' ORDER BY kh.IDKH DESC;";
		$result = $kn->query($query);
		return $result;
	}
	function laykhoahoc_all(){
		$kn = new clsKetnoi();
